...
(function ($) {
    ...
    TagsInput.prototype = {
      ...
      add: function(item, dontPushVal, options) {
        ...
        // Throw an error when trying to ad an object while the itemValue option was not set
        if (typeof item === "object" && !self.objectItems)
          throw("Can't ad objects when itemValue option is not set");
        ...
        // ad <option /> if item represents a value not present in one of the <select />'s options
      ...
      build: function(options) {
        ...
        self.$container.on('keypress', 'input', $.proxy(function(event) {
           ...
           if (self.options.freeInput && (keyCombinationInList(event, self.options.confirmKeys) || maxLengthReached)) {
              // Only attempt to ad a tag if there is data in the field
              if (text.length !== 0) {
        ...
        // Only ad existing value as tags when using strings as tags
